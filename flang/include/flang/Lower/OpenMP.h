...
/// (the caller then falls back to \p base). \p base is expected to have variant
/// entries (see SubprogramDetails::ompDeclareVariants).
const Fortran::semantics::Symbol *resolveDeclareVariantCallee(
    const Fortran::semantics::Symbol &base, AbstractConverter &converter);
} // namespace omp
