...
const semantics::Symbol *resolveDeclareVariantCallee(
    const semantics::Symbol &base, AbstractConverter &converter) {
  const semantics::Symbol &ultimate{base.GetUltimate()};
  // Callers are expected to have already checked that declare-variant entries
  // exist (via ompDeclareVariants()), so assert that here.
  ...
  // Match descriptors are not stored on the symbol (that would couple symbol.h
  // to the LLVM OpenMP context types). Rebuild each one here from the parsed
  // MATCH-clause selector recorded during semantics. A constant user condition
  // is required by semantics, so the returned dynamic condition is unused.
  //
  // There is one SemanticsContext per compilation, reachable from any symbol
  // via its owning scope; use that rather than threading it through the
  // AbstractConverter interface for this single caller.
