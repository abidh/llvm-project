...
void ModuleVisitor::Post(const parser::UseStmt &x) {
  ...
  if (const auto *onlyList{std::get_if<std::list<parser::Only>>(&x.u)}) {
    ...
    if (renameList->empty()) {
      ...
      PreservedUseStmt stmt{
          moduleName, isIntrinsic, PreservedUseStmt::Kind::UseAll};
      currScope().add_preservedUseStmt(std::move(stmt));
    } else {
      ...
      for (const auto &rename : *renameList) {
        common::visit(
            common::visitors{
                [&](const parser::Rename::Names &names) {
                  std::string localName{std::get<0>(names.t).source.ToString()};
                  std::string useName{std::get<1>(names.t).source.ToString()};
                  stmt.renames[localName] = useName;
                },
                [&](const parser::Rename::Operators &) {
                  // Operator renames - not commonly needed for debug info
