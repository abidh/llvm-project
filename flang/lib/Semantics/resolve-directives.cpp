...
public:
  ...
  void Post(const parser::OmpDeclareVariantDirective &x) {
    // Record the variant on its base procedure here, during name resolution,
    // rather than in the structure checker. Placing it in name resolution keeps
    // the door open for rebuilding the data on a future module-file read-back;
    // that module-file support is a follow-up. Diagnostics live in the checker.
