...
of the metadata generated in this step will be `DILocalVariableAttr` or
` DIDerivedTypeAttr`.

2. Changes that can only happen during or after conversion to LLVM dialect. The
example of this will be passing `DIGlobalVariableExpressionAttr` while
...
This is similar to what AddDebugFoundationPass is currently doing. This seems
...
the `BlockArgument` are not hanlded by the FIRToLLVMLowering. This can easily
be handled by adding a pass that runs after conversion to LLVM dialect.
This pass looks at the block arguments for all the functions and generates
`DbgDeclareOp` where needed.
 
### Module

In debug metadata, fortran module is represented by `DIModule`. The variables
or function inside module will have scope pointing to the parent module.
...
the location where they are declared (e.g. Line 1 in the above example.)

But the information that a variable or function is part of a module
can be extracted from its mangled name alongwith name of the module. There is
...
A derived type is represented in metadata by `DICompositeType` with a tag of
`DW_TAG_structure_type`, It has elements which points to the components.
...
A common block is represented in metadata by `DICommonBlock`. This entity is
used by the variable as scope. `DIExpression` can be used to give the offset
of any given variable inside the global storage for common block.
...
array representation for asumed shape array with the following difference.
...
the case where the DeclareOp of one variable points to the DeclareOp of another
variable (e.g. a => b).
...
Some metadta types that are needed for fortran are present in LLVM IR but are
...
- LLVM LIT tests will be added to test:
  - the driver and ensure that it passes the linetable and full debug
