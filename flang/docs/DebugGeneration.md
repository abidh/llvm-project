...
Cons:
1. Passing a lot of information from the driver as disucssed in the line table
...
may needs a change. Although the metadata that we generate remains the same in
both cases.
...
In debug metadata, fortran module is represented by `DIModuleAttr`. The variables
