...
! Variant resolution driven by `device={kind(...)}` selectors under host
! compilation (no -fopenmp-is-target-device).  Mirrors the equivalent
! metadirective-device-kind.f90 tests to verify the same OMPContext rules
! apply to declare-variant callee resolution.

! kind(host) matches on a host compilation: the variant replaces the base call.
