...
TEST(CodeExtractor, AddrSpaceCastAllocaExplicitlyExcluded) {
  ...
  // Explicitly exclude tid.cast from being sunk (simulating OpenMP runtime
  // allocas)
  ...
  // tid.addr should NOT be in SinkingCands because its addrspacecast is
  // excluded
...
TEST(CodeExtractor, AddrSpaceCastMultipleAllocasWithAddrspacecast) {
  ...
  // All three allocas should be in SinkingCands since all uses are in the
  // region
